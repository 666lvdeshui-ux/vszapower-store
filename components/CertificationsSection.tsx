    ...
    <section id="certifications" style={{ padding: '80px 24px', maxWidth: '1280px', margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '48px' }}>
        <span className="badge badge-green" style={{ marginBottom: '12px', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
          <ShieldCheck size={14} /> {sectionText('INTERNATIONAL COMPLIANCE & SAFETY', lang)}
        </span>
        <h2 style={{
          ...
          {sectionText('Global Safety & Transport Certifications', lang)}
        </h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', maxWidth: '680px', margin: '12px auto 0', lineHeight: 1.6 }}>
          {sectionText('All VSZAPOWER charger docks and rechargeable button cells carry official international safety certifications required for Amazon FBA listing, customs clearance, and wholesale distribution.', lang)}
        </p>
      </div>
      ...
      <div style={{
        ...
        {CERTIFICATIONS_LIST.map((cert) => (
          <div
            ...
            role="button"
            tabIndex={0}
            aria-label={sectionText(cert.title, lang)}
            onKeyDown={(event) => {
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.06)', fontSize: '0.8rem' }}>
              <span style={{ color: 'var(--accent-green)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                <CheckCircle2 size={14} /> {sectionText(cert.verifiedStatus, lang)}
              </span>
              <span style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                {sectionText('View Preview', lang)} <ExternalLink size={12} />
              </span>
            </div>
          </div>
        ))}
      ...
      {activeCert && (
        <div style={{
          ...
          <div role="dialog" aria-modal="true" aria-label={sectionText(activeCert.title, lang)} className="glass-panel mobile-bottom-sheet-content" style={{
            width: '100%',
            maxWidth: '480px',
            padding: '32px',
            borderRadius: '20px',
            ...
            <button
              ...
              aria-label={sectionText('Close certification preview', lang)}
              style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
              ...
              <span className="badge badge-green" style={{ marginBottom: '8px' }}>{sectionText(activeCert.code, lang)}</span>
              <h3 style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--text-main)' }}>{sectionText(activeCert.title, lang)}</h3>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '12px', marginBottom: '20px', fontSize: '0.88rem', lineHeight: 1.6, color: 'var(--text-muted)' }}>
              <p style={{ marginBottom: '8px' }}><strong>{sectionText('Target Scope', lang)}:</strong> {sectionText(activeCert.scope, lang)}</p>
              <p style={{ marginBottom: '8px' }}><strong>{sectionText('Inspection Agency', lang)}:</strong> {sectionText('International Accredited Testing Lab', lang)}</p>
              <p style={{ marginBottom: '8px' }}><strong>{sectionText('Status', lang)}:</strong> {sectionText('Verified & Active for 2026 Export', lang)}</p>
              <p>{sectionText(activeCert.description, lang)}</p>
            </div>
            ...
            >
              <FileCheck size={16} /> {sectionText('Request Official Certification PDF Copies', lang)}
            </button>
          </div>
        </div>
      )}
